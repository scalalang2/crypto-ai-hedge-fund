...
public static class SharedUtils
{
    public static async Task<string> GetCurrentPositionPrompt(IUpbitClient upbitClient, List<string> availableMarket)
    {
        ...
        sb.AppendLine("Market | Amount | Avg Price | Buying Fee | Selling Fee");
        var totalKrw = 0d;
        foreach (var market in availableMarket)
        {
            ...
            sb.AppendLine($"{market} | {response.ask_account.balance} | {response.ask_account.avg_buy_price} | {response.bid_fee} | {response.ask_fee}");
